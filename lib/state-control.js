...
obj.saveRoomState = function(room, stateId) {
	...
	Homey.manager('settings').set('roomState'+room.id, stateId);
	return true;
}

obj.getRoomState = function(room) {
	if (room == null) return null;
	var stateId = Homey.manager('settings').get('roomState' + room.id);
...
obj.getStateGroupsForAutocomplete = function(searchString) {
	var stateGroups = this.getStateGroups();
	var items = [];
	if (stateGroups != null)
		for (var idx = 0; idx < stateGroups.length; idx++)
			if (this.matchesSearchString(stateGroups[idx].description, searchString))
				items[items.length] = {
...
obj.getRoomsForAutocomplete = function(stateGroupId, searchString) {
	var rooms = this.getRooms();
	var items = [];
	if (rooms != null)
		for (var idx = 0; idx < rooms.length; idx++)
			if ((stateGroupId == null) || (stateGroupId == rooms[idx].stateGroupId) && this.matchesSearchString(rooms[idx].description, searchString))
				items[items.length] = {
					roomId: rooms[idx].id,
...
obj.getStatesForAutocomplete = function(stateGroupId, searchString) {
	var states = this.getStates();
	var items = [];
	if (states != null)
		for (var idx = 0; idx < states.length; idx++)
			if ((stateGroupId == null) || (stateGroupId == states[idx].stateGroupId) && this.matchesSearchString(states[idx].description, searchString))
				items[items.length] = {
...
obj.getTriggerableActionsForAutocomplete = function(stateGroupId, searchString) {
	var actions = this.getActions();
	var items = [];
	if (actions != null)
		for (var idx = 0; idx < actions.length; idx++)
			if (actions[idx].isTriggerable && ((stateGroupId == null) || (stateGroupId == actions[idx].stateGroupId)) && this.matchesSearchString(actions[idx].description, searchString))
				items[items.length] = {
...
obj.getPerformableActionsForAutocomplete = function(stateGroupId, searchString) {
	var actions = this.getActions();
	var items = [];
	if (actions != null)
		for (var idx = 0; idx < actions.length; idx++)
			if (actions[idx].isPerformable && ((stateGroupId == null) || (stateGroupId == actions[idx].stateGroupId)) && this.matchesSearchString(actions[idx].description, searchString))
				items[items.length] = {
					actionId: actions[idx].id,
					name: actions[idx].description
				};
