...
function StateControl(){
	...
	this._settings = this.getAndFixSettings(this._settingsKey);
}
...
obj.getIndexOfItemInArray = function(sourceArray, itemId) {
	if ((itemId == null) || (itemId.length === 0)) return null;
	if ((sourceArray == null) || (sourceArray.length === 0)) return null;
	for (var idx = 0; idx < sourceArray.length; idx++)
		if (sourceArray[idx].id == itemId)
...
obj.getAndFixSettings = function(settingsKey) {
...
obj.sortObjects = function(settings) {
	var sc = this;
	
	try {
		var sortByGroupIndex = function(a, b) {
			var groupIndexA = sc.getIndexOfItemInArray(settings.groups, a.groupId);
			var groupIndexB = sc.getIndexOfItemInArray(settings.groups, b.groupId);
			return groupIndexB - groupIndexA;
		};
		
		var sortedSections = settings.sections.sort(sortByGroupIndex);
		if ((sortedSections != null) && (sortedSections.length > 0))
			settings.sections = sortedSections;
		
		var sortedStates = settings.states.sort(sortByGroupIndex);
		if ((sortedStates != null) && (sortedStates.length > 0))
			settings.states = sortedStates;
		
		var sortedFlowActions = settings.flowActions.sort(sortByGroupIndex);
		if ((sortedFlowActions != null) && (sortedFlowActions.length > 0))
			settings.flowActions = sortedFlowActions;
		
		var sortedFlowTriggers = settings.flowTriggers.sort(sortByGroupIndex);
		if ((sortedFlowTriggers != null) && (sortedFlowTriggers.length > 0))
			settings.flowTriggers = sortedFlowTriggers;
	} catch(exception) {
		Homey.log('Error in sorting of objects: ', exception);
	}
